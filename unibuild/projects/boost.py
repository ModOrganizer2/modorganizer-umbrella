...
def patchboost(context):
    try:
        savedpath = os.getcwd()
        os.chdir(
            os.path.join("{}/boost_{}".format(config["paths"]["build"], config["boost_version"].replace(".", "_"))))
        pset = patch.fromfile(os.path.join(config["paths"]["build"], "usvfs", "patches", "type_traits_vs15_fix.patch"))
        pset.apply()
        pset = patch.fromfile(os.path.join(config['__Umbrella_path'], "Patches", "boost_msvc2017_fix.patch"))
        pset.apply()
        os.chdir(savedpath)
        return True
    except OSError:
